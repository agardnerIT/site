...
The series is intended to build upon the previous lessons, so I highly suggest you complete part one ([automated host groups](assured-host-groups-autonomous-monitoring.md)) first.

![](../images/postimages/host-metadata-1.png)

## Include Metadata in JSON

As a reminder, we ended the [previous tutorial](assured-host-groups-autonomous-monitoring.md) with a JSON file which looked like this:
